...
export default function Production() {
    ...
    return (
        <section
            id="process-section"
            className="py-12 sm:py-16 lg:py-20 bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 relative overflow-hidden"
        >
            {/* Background Elements - Ridotti su mobile */}
            <div className="absolute inset-0">
                <div className="absolute top-1/4 left-1/4 w-48 h-48 md:w-96 md:h-96 bg-indigo-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse"></div>
                <div className="absolute bottom-1/4 right-1/4 w-48 h-48 md:w-96 md:h-96 bg-blue-200 rounded-full mix-blend-multiply filter blur-3xl opacity-20 animate-pulse animation-delay-2000"></div>
            </div>
            ...
            <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div className="text-center mb-12 sm:mb-16">
                    <span className="inline-block bg-indigo-100 text-indigo-800 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-medium mb-4">
                        🔧 Il Nostro Metodo
                    </span>
                ...
                <div className="relative">
                    {/* Central Line - Solo desktop */}
                    <div className="hidden xl:block absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-indigo-200 via-purple-300 to-indigo-200"></div>
                        ...
                        {processSteps.map((step, index) => (
                            ...
                            >
                                ...
                                <div className="xl:hidden">
                                    <div className={`${activeStep === index ? 'scale-105' : ''} transition-transform duration-500`}>
                                        <div className={`bg-white rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-1 
                                            ${activeStep === index ? 'ring-2 sm:ring-4 ring-indigo-300 shadow-2xl' : ''}`}
                                        >
                                            <div className="flex items-start space-x-4">
                                                {/* Icon e numero */}
                                                <div className="flex-shrink-0">
                                                    <div className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 bg-gradient-to-r ${step.gradient} rounded-xl sm:rounded-2xl flex items-center justify-center text-white 
                                                        ${activeStep === index ? 'scale-110' : ''} transition-transform duration-300 relative`}
                                                    >
                                                        {step.icon}
                                                        <div className="absolute -top-2 -left-2 w-6 h-6 sm:w-8 sm:h-8 bg-white rounded-full shadow-lg flex items-center justify-center border-2 border-indigo-100">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="mb-3 sm:mb-4">
                                                        <span className="text-xs sm:text-sm font-medium text-indigo-600 mb-1 sm:mb-2 block">
                                                            Step {step.id} • {step.duration}
                                                        </span>
                                                        <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-1 sm:mb-2">
                                                            {step.title}
                                                        </h3>
                                                        <p className="text-sm sm:text-base md:text-lg text-indigo-600 font-medium mb-2 sm:mb-4">
                                                            {step.subtitle}
                                                        </p>
                                                    </div>
                                                    ...
                                                    {/* Details - Solo su schermi più grandi o quando step è attivo */}
                                                    <div className={`transition-all duration-300 ${activeStep === index || window.innerWidth >= 768 ? 'block' : 'hidden'}`}>
                                                        <ul className="space-y-1 sm:space-y-2 mb-4 sm:mb-6">
                                                            {step.details.map((detail, idx) => (
                                                                <li key={idx} className="flex items-start text-xs sm:text-sm text-gray-600">
                                                                    <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-indigo-400 rounded-full mr-2 sm:mr-3 flex-shrink-0 mt-1.5"></span>
                                                        ...
                                                        <div className={`w-full bg-gray-200 rounded-full h-1.5 sm:h-2 ${
                                                            ...
                                                            <div
                                                                className={`bg-gradient-to-r ${step.gradient} h-1.5 sm:h-2 rounded-full transition-all duration-1000 ${
                                    ...
                                    {index < processSteps.length - 1 && (
                                        <div className="flex justify-center mt-6 sm:mt-8">
                                            <div className="w-0.5 sm:w-1 h-8 sm:h-12 bg-gradient-to-b from-indigo-300 to-purple-300 rounded-full"></div>
                                        </div>
                                    )}
                                </div>

                                {/* Desktop Layout (xl e superiori) */}
                                <div className={`hidden xl:grid xl:grid-cols-2 gap-8 items-center ${
                                    index % 2 === 0 ? '' : 'xl:grid-flow-col-dense'
                                }`}>

                                    {/* Content */}
                                    <div className={`${index % 2 === 0 ? 'xl:pr-12' : 'xl:pl-12 xl:col-start-2'} 
                                        ${activeStep === index ? 'xl:scale-105' : ''} transition-transform duration-500`}>
                                    ...
                                    {/* Visual */}
                                    <div className={`${index % 2 === 0 ? 'xl:pl-12' : 'xl:pr-12 xl:col-start-1'} 
                                        ${activeStep === index ? 'xl:scale-105' : ''} transition-transform duration-500`}>
                                        <div className="relative">
                ...
                <div className="mt-12 sm:mt-16 lg:mt-20">
                    <div className="bg-white rounded-xl sm:rounded-2xl p-6 sm:p-8 shadow-xl max-w-4xl mx-auto">
                        <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900 mb-3 sm:mb-4 text-center px-4">
                            Pronto a Iniziare il Tuo Progetto?
                        </h3>
                        <p className="text-sm sm:text-base lg:text-xl text-gray-600 mb-6 sm:mb-8 text-center px-4 max-w-2xl mx-auto">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center px-4">
                            <button className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-full font-semibold text-sm sm:text-base lg:text-lg shadow-xl hover:shadow-2xl transform hover:scale-105 transition-all duration-300">
                            ...
                            <a
                                ...
                                className="border-2 border-indigo-600 text-indigo-600 px-6 sm:px-8 py-3 sm:py-4 rounded-full font-semibold text-sm sm:text-base lg:text-lg hover:bg-indigo-600 hover:text-white transition-all duration-300 transform hover:scale-105"
                            >
                                <span className="flex items-center justify-center">
                        ...
                        <div className="flex items-center justify-center mt-4 sm:mt-6 text-xs sm:text-sm text-gray-500">
