...
export default function Preventivo() {
    ...
    const handleChange = (e) => {
        ...
        if (type === 'checkbox') {
            if (name === 'finiture') {
                setFormData(prev => ({ ...prev, finiture: checked ? [...prev.finiture, value] : prev.finiture.filter(f => f !== value) }));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.keys(formData).forEach(key => {
                if (key === 'uploadedFile' && formData[key]) { formDataToSend.append('file', formData[key].file); formDataToSend.append('fileName', formData[key].name); }
                else if (key === 'finiture') { formDataToSend.append(key, JSON.stringify(formData[key])); }
                else if (key !== 'uploadedFile') { const value = formData[key]; if (value !== null && value !== undefined) formDataToSend.append(key, value); }
            });
            ...
            if (response.ok && result.success) {
                ...
                setFormData({ nome:'',cognome:'',email:'',telefono:'',azienda:'',tipoProgetto:'',servizio:'',urgenza:'',quantita:'',formato:'',pagine:'',colori:'',carta:'',finiture:[],hasFile:'',fileInfo:'',uploadedFile:null,note:'',budget:'',privacy:false,newsletter:false });
                setCurrentStep(1);
    ...
    const formatoOpzioni = ['A4 (21x29.7 cm)','A5 (14.8x21 cm)','A3 (29.7x42 cm)','A6 (10.5x14.8 cm)','10x15 cm','13x18 cm','Formato personalizzato','Grande formato'];
    const cartaOpzioni = ['Carta normale 80g','Carta patinata lucida 130g','Carta patinata opaca 130g','Carta patinata lucida 170g','Carta patinata opaca 170g','Carta patinata lucida 250g','Carta patinata opaca 250g','Cartoncino 300g','Carta riciclata','Carta speciale'];
    const finitureOpzioni = [
        {id:'plastificazione-lucida',name:'Plastificazione Lucida'},{id:'plastificazione-opaca',name:'Plastificazione Opaca'},
        {id:'verniciatura-uv',name:'Verniciatura UV Selettiva'},{id:'stampa-oro',name:'Stampa Oro a Caldo'},
        {id:'stampa-argento',name:'Stampa Argento a Caldo'},{id:'rilievo-secco',name:'Rilievo a Secco'},
        {id:'fustellatura',name:'Fustellatura'},{id:'cordonatura',name:'Cordonatura'},{id:'rilegatura',name:'Rilegatura'}
    ];
        ...
        <div className="min-h-screen bg-gradient-to-br from-teal-50 via-white to-cyan-50">
            ...
            <div className="bg-gradient-to-r from-teal-600 via-cyan-600 to-teal-700 text-white py-16">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                    ...
                    <p className="text-xl text-teal-100 mb-8">Compila il form per ricevere un preventivo dettagliato e gratuito entro 24 ore</p>
                    <div className="max-w-lg mx-auto">
                        <div className="flex justify-between items-center mb-4">
                            {[1,2,3,4].map((step) => (
                                <div key={step} className="flex items-center">
                                    ...
                                    {step < 4 && (<div className={`w-16 h-1 mx-2 rounded-full transition-all duration-500 ${step < currentStep ? 'bg-white' : 'bg-teal-400'}`}></div>)}
                                </div>
                            ))}
                        </div>
                        ...
                        <p className="text-teal-100 text-sm mt-2">Step {currentStep} di 4</p>
                    </div>
                </div>
            </div>
                ...
                <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
                    <form onSubmit={handleSubmit}>
                        ...
                        {currentStep === 1 && (
                            <div className="p-8">
                                <div className="text-center mb-8">
                                    <div className="w-16 h-16 bg-gradient-to-r from-teal-500 to-cyan-500 rounded-2xl flex items-center justify-center text-2xl text-white mx-auto mb-4">👤</div>
                        ...
                        {currentStep === 2 && (
                            <div className="p-8">
                                <div className="text-center mb-8">
                                    <div className="w-16 h-16 bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl flex items-center justify-center text-2xl text-white mx-auto mb-4">🎯</div>
                                    <h2 className="text-3xl font-bold text-gray-900 mb-4">Che Cosa Vuoi Stampare?</h2>
                                    <p className="text-gray-600">Seleziona il tipo di progetto per ricevere consigli personalizzati</p>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                                    {tipiProgetto.map((tipo) => (
                                        <div key={tipo.id}
                                             className={`p-6 rounded-xl border-2 cursor-pointer transition-all duration-300 hover:shadow-lg ${formData.tipoProgetto === tipo.id ? 'border-teal-500 bg-teal-50 shadow-lg transform scale-105' : 'border-gray-200 hover:border-teal-300'}`}
                                            ...
                                            <ul className="text-sm text-gray-600 space-y-1">
                                                {tipo.items.slice(0,3).map((item,idx) => <li key={idx} className="flex items-center"><span className="w-1 h-1 bg-teal-400 rounded-full mr-2"></span>{item}</li>)}
                                                {tipo.items.length > 3 && <li className="text-teal-600 font-medium">e altro...</li>}
                                            </ul>
                                        </div>
                                    ))}
                                </div>
                                {formData.tipoProgetto && (
                                    <div className="space-y-6">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-4">Seleziona il servizio specifico *</label>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                {servizi.map((servizio) => (
                                                    <div key={servizio.id}
                                                         className={`p-4 rounded-lg border cursor-pointer transition-all duration-200 ${formData.servizio === servizio.id ? 'border-teal-500 bg-teal-50' : 'border-gray-200 hover:border-teal-300'}`}
                                                         onClick={() => setFormData(prev => ({...prev, servizio: servizio.id}))}>
                                                        <div className="flex items-center space-x-3">
                                                            <span className="text-xl">{servizio.icon}</span>
                                                            <div><h4 className="font-medium text-gray-900">{servizio.name}</h4><p className="text-sm text-gray-600">{servizio.desc}</p></div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Urgenza del progetto *</label>
                                            <select name="urgenza" required value={formData.urgenza} onChange={handleChange} className={inputClass}>
                        ...
                        {currentStep === 3 && (
                            <div className="p-8">
                                <div className="text-center mb-8">
                                    <div className="w-16 h-16 bg-gradient-to-r from-teal-500 to-cyan-600 rounded-2xl flex items-center justify-center text-2xl text-white mx-auto mb-4">⚙️</div>
                                    <h2 className="text-3xl font-bold text-gray-900 mb-4">Dettagli Tecnici</h2>
                                    <p className="text-gray-600">Specifica le caratteristiche tecniche per un preventivo preciso</p>
                                </div>
                                <div className="space-y-6">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div><label className="block text-sm font-medium text-gray-700 mb-2">Quantità *</label><input type="number" name="quantita" required min="1" value={formData.quantita} onChange={handleChange} className={inputClass} placeholder="500" /></div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Formato *</label>
                                            <select name="formato" required value={formData.formato} onChange={handleChange} className={inputClass}>
                                                <option value="">Seleziona formato</option>
                                                {formatoOpzioni.map((f,i) => <option key={i} value={f}>{f}</option>)}
                                            </select>
                                        </div>
                                        <div><label className="block text-sm font-medium text-gray-700 mb-2">Numero di pagine (se applicabile)</label><input type="number" name="pagine" min="1" value={formData.pagine} onChange={handleChange} className={inputClass} placeholder="8" /></div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Colori *</label>
                                            <select name="colori" required value={formData.colori} onChange={handleChange} className={inputClass}>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Tipo di carta *</label>
                                        <select name="carta" required value={formData.carta} onChange={handleChange} className={inputClass}>
                                            <option value="">Seleziona tipo di carta</option>
                                            {cartaOpzioni.map((c,i) => <option key={i} value={c}>{c}</option>)}
                                        </select>
                                    </div>
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-4">Finiture speciali (opzionale)</label>
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                            {finitureOpzioni.map((finitura) => (
                                                <label key={finitura.id} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                                                    <input type="checkbox" name="finiture" value={finitura.id} checked={formData.finiture.includes(finitura.id)} onChange={handleChange} className="w-4 h-4 text-teal-600 border-gray-300 rounded focus:ring-teal-500" />
                        ...
                        {currentStep === 4 && (
                            <div className="p-8">
                                <div className="text-center mb-8">
                                    <div className="w-16 h-16 bg-gradient-to-r from-orange-500 to-red-600 rounded-2xl flex items-center justify-center text-2xl text-white mx-auto mb-4">📎</div>
                                    <h2 className="text-3xl font-bold text-gray-900 mb-4">File e Informazioni Aggiuntive</h2>
                                    <p className="text-gray-600">Completa con eventuali file e note per finalizzare la richiesta</p>
                                </div>
                                <div className="space-y-6">
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Hai già i file pronti?</label>
                                        <div className="space-y-4">
                                            <label className="flex items-center space-x-3"><input type="radio" name="hasFile" value="true" checked={formData.hasFile === 'true'} onChange={handleChange} className="w-4 h-4 text-teal-600 border-gray-300 focus:ring-teal-500" /><span>Sì, ho i file pronti</span></label>
                                            <label className="flex items-center space-x-3"><input type="radio" name="hasFile" value="false" checked={formData.hasFile === 'false'} onChange={handleChange} className="w-4 h-4 text-teal-600 border-gray-300 focus:ring-teal-500" /><span>No, ho bisogno di aiuto per la progettazione</span></label>
                                        </div>
                                    </div>
                                    <FileUploadComponent onFileChange={handleFileChange} hasFile={formData.hasFile} />
                                    {formData.hasFile === 'true' && !formData.uploadedFile && (
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-2">Informazioni sui file</label>
                                            <textarea name="fileInfo" value={formData.fileInfo} onChange={handleChange} rows={3} className={`${inputClass} resize-none`} placeholder="Descrivi i file che hai (formato, risoluzione, etc.)"></textarea>
                                            <p className="text-sm text-gray-500 mt-2">💡 Potrai inviarci i file via email o WhatsApp dopo aver ricevuto il preventivo</p>
                                        </div>
                                    )}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">Budget indicativo (opzionale)</label>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="note" value={formData.note} onChange={handleChange} rows={4} className={`${inputClass} resize-none`} placeholder="Aggiungi qualsiasi informazione aggiuntiva..."></textarea>
                                    </div>
                                    <div className="space-y-4 pt-6 border-t border-gray-200">
                                        ...
                                        <label className="flex items-start space-x-3">
                                            ...
                                            <span className="text-sm text-gray-700">Desidero ricevere offerte speciali e novità via email</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center space-x-4">
                                {currentStep < 4 ? (
                                    <button type="button" onClick={nextStep}
                                            disabled={isSubmitting || (currentStep===1 && (!formData.nome||!formData.cognome||!formData.email||!formData.telefono)) || (currentStep===2 && (!formData.tipoProgetto||!formData.servizio||!formData.urgenza)) || (currentStep===3 && (!formData.quantita||!formData.formato||!formData.colori||!formData.carta))}
                                    ...
                                    <button type="submit" disabled={!formData.privacy || isSubmitting}
